...
namespace ChessMainLoop
{
    public class PieceController : Singleton<PieceController>
    {
        ...
        {
            Piece selectedPiece = _activePiece;
            Vector3 targetPosition = new Vector3(newRow * BoardState.Offset, selectedPiece.transform.localPosition.y, newColumn * BoardState.Offset);

            AnimationManager.Instance.MovePiece(selectedPiece, targetPosition, null);
            ...
            {
                BoardState.Instance.ClearField(assignedEnemy.Location.Row, assignedEnemy.Location.Column);
                assignedEnemy.gameObject.SetActive(false);
            }
            ...
            BoardState.Instance.SetField(selectedPiece, newRow, newColumn);
            selectedPiece.Move(newRow, newColumn);
            ...
            SideColor gameOverSide = BoardState.Instance.CheckIfGameOver();
            if (gameOverSide != SideColor.None)
            {
                GameManager.Instance.GameEnd(gameOverSide);
                yield break;
            }
